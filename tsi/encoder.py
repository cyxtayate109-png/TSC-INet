...
class Encoder(nn.Module):
    def __init__(self, hidden_size, num_head, num_layer, gcn_base_channel=32) -> None:
        super().__init__()
        self.d_model = hidden_size
        self.gcn_base_channel = gcn_base_channel

        # Use configurable GCN base channel for lightweight model
        self.gcn_t = Model(gcn_base_channel)
        self.gcn_s = Model(gcn_base_channel)

        # Temporal channel: 50 joints * M persons * gcn_channel
        self.channel_t = nn.Sequential(
            nn.Linear(50*gcn_base_channel, self.d_model),
            nn.LayerNorm(self.d_model),
            nn.ReLU(True),
            nn.Linear(self.d_model, self.d_model),
        )

        # Spatial channel: 64 frames * gcn_channel
        self.channel_s = nn.Sequential(
            nn.Linear(64 * gcn_base_channel, self.d_model),
            nn.LayerNorm(self.d_model),
            nn.ReLU(True),
            nn.Linear(self.d_model, self.d_model),
        )
...
class PretrainingEncoder(nn.Module):
    def __init__(self, hidden_size, num_head, num_layer,
        ...
        self.encoder = Encoder(
            hidden_size, num_head, num_layer, gcn_base_channel=gcn_base_channel,
        )
