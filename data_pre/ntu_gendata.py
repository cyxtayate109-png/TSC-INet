...
def gendata(data_path, out_path, ignored_sample_path=None, benchmark='xview', part='eval'):
    ...
    sample_name = []
    sample_label = []
    subdirectories = [os.path.join(data_path, sub_dir) for sub_dir in os.listdir(data_path) if
                      os.path.isdir(os.path.join(data_path, sub_dir))]
    for sub_dir in subdirectories:
        for filename in os.listdir(sub_dir):
            full_filename = os.path.join(sub_dir, filename)
            if full_filename in ignored_samples:
                continue
            action_class = int(
                filename[filename.find('A') + 1:filename.find('A') + 4])
            subject_id = int(
            ...
            if issample:
                sample_name.append(full_filename)
                sample_label.append(action_class - 1)
    ...
    for i, s in enumerate(tqdm(sample_name)):
        data = read_xyz(os.path.join(s), max_body=max_body_kinect, num_joint=num_joint)
        fp[i, :, 0:data.shape[1], :, :] = data
        fl[i] = data.shape[1]  # num_frame
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--data_path', default='../data/nturgbd_sam/nturgb+d_skeletons/')
    ...
    parser.add_argument('--data_path', default='../data/nturgbd_sam_120/nturgb+d_skeletons/')
    parser.add_argument('--ignored_sample_path',
